<template>
  <div class="watchlist-page">
    ...
    <div class="watchlist-hero">
      <div class="hero-text">
        <h1 class="hero-title">自选股</h1>
        <p class="hero-subtitle">管理自选股分组 · 监控时快速选择</p>
      </div>
    </div>
...
const createInputRef = ref<InstanceType<typeof import('element-plus')['ElInput']> | null>(null)

function showCreateGroup() {
  creatingGroup.value = true
  newGroupName.value = ''
  nextTick(() => {
    const inputs = document.querySelectorAll('.group-item.creating .el-input__inner') as NodeListOf<HTMLInputElement>
...
function startRename(g: WatchlistGroup) {
  ...
  nextTick(() => {
    const inputs = document.querySelectorAll('.group-item .el-input__inner') as NodeListOf<HTMLInputElement>
    if (inputs.length) inputs[0].focus()
  })
}
...
async function handleAddStocks() {
  ...
  try {
    ...
    ElMessage.success(`已添加 ${data.added} 只股票${data.skipped > 0 ? `，跳过 ${data.skipped} 只重复` : ''}`)
